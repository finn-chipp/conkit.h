...
// Versions for non-lvalue arguments:
...
// Note: this is not a problem for the lvalue-argument versions, since they do not require a buffer be written-to
...
// Versions that should be used when the specified arguments are lvalues:
...
void ck_print(char *buffer) { // Write a string to the CK_SCREEN_BUFFER
    ...
    if((CK_SCREEN_BUFFER = realloc(CK_SCREEN_BUFFER, CK_SCREEN_BUFFER_SIZE * sizeof(char))) == NULL) { // Reallocate
